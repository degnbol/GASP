#!/usr/bin/env zsh
mlr --icsv --otsv --from `git root`/data/lit/raw/sebastian/gt_all_data.csv join -j sugar_acceptor -f `git root`/data/lit/raw/sebastian/gt_acceptors_unique_CIDmanual.csv then \
